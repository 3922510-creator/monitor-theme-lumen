 ...
 * underneath. Unlike the default theme's monochrome bar, the fill is tinted by
 * how full it is -- calm indigo while there is room, amber as it tightens, red
 * once it is nearly full -- so a loaded node reads at a glance without opening it.
 */

// The thresholds a utilisation crosses. CPU, memory, disk and this-period
// traffic all read "more is worse", so one scale serves every meter.
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-muted">
        <div
          className={cn("h-full rounded-full transition-[width,background-color] duration-500", pct === null ? "bg-transparent" : tone(filled))}
          style={{ width: `${filled}%` }}
        />
      </div>
