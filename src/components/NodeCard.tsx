...
export function Status({ node }: { node: Node }) {
  ...
  return (
    ...
    >
      {/* A live node gets a green dot with a soft halo; an absent one a flat
          grey. Colour rather than the default theme's monochrome fill, since a
          "在线" that reads as reassuring is the first thing a status page owes. */}
      <span className={cn("size-1.5 rounded-full", node.online ? "bg-ok shadow-[0_0_0_3px] shadow-ok/20" : "bg-muted-foreground/40")} />
      {label.trim()}
    </Badge>
  )
}
...
export function NodeCard({ node, onOpen }: { node: Node; onOpen: () => void }) {
  ...
  return (
    <Card
      ...
      className="min-w-0 cursor-pointer gap-0 p-4 transition-[colors,box-shadow] hover:border-primary/40 hover:shadow-md"
      role="button"
      tabIndex={0}
